...
def train(attn_implementation=None):
    ...
    projector_ckpt_dir = 'projector_ckpt_dir_path/' # TODO: set the projector checkpoint dir path
    
    ckpt = torch.load(join(projector_ckpt_dir,'audio_pretrain.bin'),map_location='cpu')
    weight = ckpt.pop('model.embed_tokens.weight')
    model.model.load_state_dict(ckpt,strict=False)
    print(f'audio: pop embed weight, shape: {weight.shape}')
    
    ckpt = torch.load(join(projector_ckpt_dir,'visual_pretrain.bin'),map_location='cpu')
    weight = ckpt.pop('model.embed_tokens.weight')
    model.model.load_state_dict(ckpt,strict=False)
    print(f'visual: pop embed weight, shape: {weight.shape}')
